...
// 꾸미기 액세서리 — 16x16 오버레이(국장 스프라이트와 동일 정렬). 투명(".") 제외 픽셀만 국장 위에 얹힌다.
// 머리(0~2행)·눈(5행, 눈은 col4·col9)·코입(6~7행) 위치 기준으로 디자인.
export const COSMETIC_MAPS = {
  "cos-tophat": [ // 검은 중절모
    "....kkkkkk......",
    "...kkkkkkkk.....",
    "..kkkkkkkkkk....",
  ...
  // ── 캐릭터 시그니처(선택 캐릭터 고유 외형) ──
  "sig-band": [ // 열혈 국장 — 빨간 머리띠(이마)
    "................", "................",
    "..tttttttttt....",
    "..tttttttttt....",
    "................", "................", "................", "................",
...
// create() 시점에 1회 호출 — 국장 5단계 텍스처(gov-1..5).
export function buildGovTextures(scene) {
  GOV_MAPS.forEach((map, i) => bakeTexture(scene, `gov-${i + 1}`, map));
}

// 꾸미기 액세서리 오버레이 텍스처(cos-*) 1회 굽기.
export function buildCosmeticTextures(scene) {
  Object.entries(COSMETIC_MAPS).forEach(([key, map]) => bakeTexture(scene, key, map));
}
