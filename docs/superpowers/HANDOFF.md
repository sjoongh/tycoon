...
## 루프22 완료 (콘텐츠 깊이 — 빌드 결정, 밤샘 자율)
- **직원×시설 시너지**(d5d6e29): 기획 4차검증 P1 "결정 깊이 부재"(직원 스킬 가산만→'제일 싼 거 사기'가 최적) 직접 해소. 6직원 각자 대응 시설과 **곱연산** 시너지 = `직원Lv × 시설Lv × 0.0004`. 매핑: clerk↔desk, auditor↔archive, runner↔sorter, engineer↔server, analyst↔notice, speaker↔studio. `staffMultiplierFor` 일반화(기존 engineer>=4?server*0.006 하드코딩 → synergy 데이터로 이전, 이제 직원 레벨에도 비례). 직원 카드에 `◈ ○○ 시너지` 표기(시설·직원 둘 다 있으면 진한 보라 활성). → "전산실 많이 키웠으니 전산관 키우자" 식 빌드 결정 발생.
- 밸런스: 초반(직원2×시설5=0.004) 무손상, 곱연산이라 둘 다 키워야 보상 큼(외길 방지). 회귀 cps 599→612(+2.2%, 의도된 시너지 보너스). e2e: engineer10×server40×0.0004=0.16 정확·초반 negligible·UI 표기. build✓ test13/13✓ 0에러.
- **남은 기획 P1**: 콘텐츠 런웨이(언락형 업그레이드 티어/2차 프레스티지 통화로 후반 목표 연장). 다음 루프 후보(신중·과개발 금지). 그 외엔 마무리 폴리시.

## 루프23 완료 (콘텐츠 런웨이 — 2차 통화, 밤샘 자율)
- **2차 프레스티지 통화 '훈장'(medal)**(9aca847): 기획 4차검증 P1 "콘텐츠 런웨이"(명명 퀘스트 후 생성형 반복만) 해소.
  - **적립**: 새 구역을 *처음* 돌파할 때만 `area - medalBest`만큼(medalBest 갱신) → 감사 스팸 불가, 깊이 진행 보상. (단순 인장 누적이 아니라 '더 멀리 가야' 얻음)
  - **훈장 업글 5종**(인장 +%/Lv 복붙이 아닌 질적 메타): 포고령(감사 인장 획득 ×(1+12%/Lv) 곱연산), 관록(영구배율 +15%/Lv), 유산(시작 표 +1200/Lv), 전당(인장 업글 maxLevel +2/Lv = 상한 해제), 명예전당(무한 cpsPct).
  - `buyPrestigeUpgrade` currency(seal/medal) 분기, `allPrestigeUpgrades`, capBoost로 인장 maxLevel 동적. 감사탭 훈장 섹션(금색, 인장 gem-blue와 구분) + 감사 모달 훈장 미리보기.
  - 순환 런웨이: 감사로 인장↑ → 새 구역 돌파로 훈장 획득 → 훈장으로 인장 경제 강화 → 더 깊이. AdVenture Capitalist의 angel investor 식 메타 레이어.
- e2e(`_medaltest.mjs`): 적립 area8→훈장4·같은구역 재감사 0·더깊은구역 추가·곱연산 인장 44→71·상한 manualMax 15→21·유산 시작표 2400·구매 차감. 회귀 cps612 0에러 build✓ test13/13✓. 초반/트러스트 무손상.
- **현황: 기획 4차검증 P1 2건(①결정 깊이=직원×시설 시너지[루프22] ②콘텐츠 런웨이=훈장[루프23]) 전부 완료.** 92%의 발목 두 가지가 메워짐.
- **다음 루프: 5차 경량 종합검증으로 95% 재판정.** P1 해소 후 종합 점수 재확인. 미달 없으면 마무리 선언.
