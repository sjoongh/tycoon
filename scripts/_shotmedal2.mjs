...
// 패널을 훈장 섹션이 보이도록 스크롤
await p.evaluate(()=>{const el=document.querySelector(".gp-panel");if(el)el.scrollTop=el.scrollHeight;
  // 훈장 칸 선택해서 디테일 카드도 노출
  const m=document.querySelector(".gp-seal--medal");if(m)m.click();});
await p.waitForTimeout(400);
await p.evaluate(()=>{const el=document.querySelector(".gp-panel");if(el)el.scrollTop=el.scrollHeight;});
await p.waitForTimeout(200);
await p.screenshot({path:"/tmp/rv-medal2.png"});
