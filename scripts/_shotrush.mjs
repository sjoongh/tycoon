...
const mk=(rush)=>({version:3,votes:5e5,explain:8000,trust:78,days:18,paused:false,selected:"desk",activeTab:"facilities",
  ...
  quests:{"first-500":true,"desk-5":true,"trust-80":true,"cps-25":true,"stage-2":true,"facility-total-30":true},endless:5,daily:{day:99999999,streak:1},eventReadyAt:now+9e8,...rush});
...
for(const [name,rush] of [["ready",{rushReadyAt:0,rushEndsAt:0}],["active",{rushReadyAt:now+9e8,rushEndsAt:now+15000}]]){
  const p=await b.newPage({viewport:{width:390,height:844},deviceScaleFactor:2});
  await p.addInitScript(([k,v])=>localStorage.setItem(k,v),[SAVE_KEY,JSON.stringify(mk(rush))]);
  await p.goto("http://localhost:5178/",{waitUntil:"domcontentloaded",timeout:20000}); await p.waitForTimeout(3500);
  await p.screenshot({path:`/tmp/rv-rush-${name}.png`}); await p.close();
}
